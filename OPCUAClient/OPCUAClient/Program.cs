...
namespace OPCUAClient
{
    class Program
    {
        ...
        {
            Console.WriteLine("OPC UA Client configuration");
            var config = new ApplicationConfiguration()
            {
                ApplicationName = "MySiteClient",
                ApplicationUri = Utils.Format(@"urn:{0}:MySiteClient", System.Net.Dns.GetHostName()),
                ApplicationType = ApplicationType.Client,
                SecurityConfiguration = new SecurityConfiguration
                {
                    ApplicationCertificate = new CertificateIdentifier { StoreType = @"Directory", StorePath = @"%CommonApplicationData%\OPC Foundation\CertificateStores\MachineDefault", SubjectName = "MySiteClient" },
            ...
            var selectedEndpoint = CoreClientUtils.SelectEndpoint("opc.tcp://hiqarbl344:26543/OPCUAServer", useSecurity: true);
